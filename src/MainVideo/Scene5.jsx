...
import { FONT_FAMILY, SCENE_5_DURATION } from "./constants";
...
export const Scene5 = () => {
  ...
  const windowY = interpolate(frame, [0, SCENE_5_DURATION], [10, -8], clamp);
  const windowScale = interpolate(frame, [0, SCENE_5_DURATION], [0.99, 1.03], clamp);
  const windowRotate = interpolate(frame, [0, SCENE_5_DURATION], [-0.15, 0.15], clamp);
    ...
    <AbsoluteFill style={{ backgroundColor: "#f8fafc", fontFamily: FONT_FAMILY }}>
      ...
      >
        ...
        <div className="relative w-full h-full bg-white flex items-start justify-center overflow-hidden">
             <Img 
                src={staticFile("screenshots/pembayaran.png")}
                style={{
                    width: "100%",
                ...
                onError={(e) => {
                    ...
                    e.target.parentNode.innerHTML = '<div class="flex items-center justify-center h-full text-gray-400">Payment Page Screenshot Placeholder</div>';
                }}
             />
        </div>
      </div>
      ...
      >
        <div className="inline-block px-10 py-4 bg-white/80 backdrop-blur-md rounded-2xl shadow-xl border border-white/50">
             <h2 className="text-3xl font-bold text-[#004080] tracking-tight">
                Make payments with <span className="text-[#fdc134]">FPX</span> or card, quickly and securely
             </h2>
        </div>
      </div>
    </AbsoluteFill>
